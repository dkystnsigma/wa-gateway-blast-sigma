<x-layout-dashboard title="Form Order - Google Sheets">
    ...
    @if (isset($info))
        <div class="alert alert-info">
            ...
            @if (isset($device_status_legend))
                <div class="mt-2">
                    <h6>Device Status Legend:</h6>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="d-flex align-items-center">
                                <div class="bg-white border rounded me-2" style="width: 20px; height: 20px;"></div>
                                <small>Normal - Device is connected</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex align-items-center">
                                <div class="bg-warning border rounded me-2" style="width: 20px; height: 20px;"></div>
                                <small>Yellow - Device exists but not connected</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex align-items-center">
                                <div class="bg-danger border rounded me-2" style="width: 20px; height: 20px;"></div>
                                <small>Red - Device not found in database</small>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <div class="btn-group btn-group-sm me-2" role="group" aria-label="Device status filter">
                            <button type="button" class="btn btn-outline-secondary active" id="filter-all" data-filter="all">
                                <i class="bi bi-funnel"></i> All
                            </button>
                            <button type="button" class="btn btn-outline-success" id="filter-connected" data-filter="connected">
                                <i class="bi bi-check-circle"></i> Connected
                            </button>
                            <button type="button" class="btn btn-outline-warning" id="filter-warning" data-filter="warning">
                                <i class="bi bi-exclamation-triangle"></i> Not Connected
                            </button>
                            <button type="button" class="btn btn-outline-danger" id="filter-danger" data-filter="danger">
                                    ...
                                    <tr class="{{ $row['row_class'] ?? '' }}">
                                        ...
                                        @if (is_array($row))
                                            @foreach ($row as $key => $value)
                                                @if ($key !== 'device_status' && $key !== 'row_class')
                                                    <td>
    ...
    @else
        <div class="text-center mt-5">
            ...
            <h4 class="mt-3">Form Order</h4>
            <p class="text-muted">Manage your form orders here</p>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <p class="text-muted small">Add your form order data starting from row 2</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (table) {
                ...
                function applyFilters() {
                    ...
                    let statusFilteredRows = allRows;
                    if (currentFilter !== 'all') {
                        statusFilteredRows = allRows.filter(row => {
                            if (row.cells.length === 1 && row.cells[0].colSpan > 1) {
                                return currentFilter === 'all'; // "No data available" row
                            }

                            const rowClass = row.className || '';
                            switch (currentFilter) {
                                case 'connected':
                                    return !rowClass.includes('table-warning') &&
                                           !rowClass.includes('table-danger') &&
                                           !rowClass.includes('table-secondary');
                                case 'warning':
                                    return rowClass.includes('table-warning');
                                case 'danger':
                                    return rowClass.includes('table-danger');
                                default:
                                    return true;
                            }
                        });
                    ...
                    filteredRows = statusFilteredRows.filter(row => {
                        if (row.cells.length === 1 && row.cells[0].colSpan > 1) {
                ...
                function setActiveFilter(button) {
                    ...
                    [filterAll, filterConnected, filterWarning, filterDanger].forEach(btn => {
                ...
                filterConnected.addEventListener('click', function() {
                    currentFilter = 'connected';
                    setActiveFilter(this);
                    applyFilters();
                });

                filterWarning.addEventListener('click', function() {
                    currentFilter = 'warning';
                    setActiveFilter(this);
                    applyFilters();
                });

                filterDanger.addEventListener('click', function() {
        ...
        function exportToCSV() {
            ...
            if (link.download !== undefined) {
                ...
                link.setAttribute('download', 'form_order.csv');
