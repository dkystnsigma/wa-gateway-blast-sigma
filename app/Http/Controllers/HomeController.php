...
class HomeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->user()->devices()->create(['name' => $request->name, 'body' => $request->sender, 'webhook' => $request->urlwebhook]);
    ...
    {
        clearCacheNode("webhook:$request->number", 3);
    ...
    public function setAvailable(Request $request)
    {
        ...
        try {
            ...
            Http::withOptions(['verify' => false])
                ->asForm()
                ->post(env('WA_URL_SERVER') . '/backend-send-available', ['body' => $request->id]);
        } catch (\Throwable $th) {
        }
